...
export interface EthereumBuildTxParams {
  actions: NearAction[];
...
export const extractGenericActionsFromTx = (
  txParams: EthereumBuildTxParams
): GenericTxAction[] => {
...
export const buildParamsFromGenericTx = (innerSdk: NearState) => async (
  ...
  const nearTxParams: EthereumBuildTxParams = {
...
export const buildNearTx = (innerSdk: NearState) => async ({
  ...
  senderAccountID,
  recipientAccountID,
}: EthereumBuildTxParams): Promise<Transaction> => {
